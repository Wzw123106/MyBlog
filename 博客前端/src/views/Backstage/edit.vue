    ...
    <div id="article">
        ...
        <div class="ar-right">
            ...
            <form action="" method="get" class="fo"> 
                <h4>权限控制</h4>
                <p><label><input name="Fruit" type="checkbox" value="允许评论" v-model="pla"/><span>允许评论</span></label></p>
                <p v-show="xs"><label><input name="Fruit" type="checkbox" value="是否置顶"  v-model="zda"/><span>是否置顶</span></label></p>
            </form>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        ok(){
            if(this.title !== "" && this.flmz !=="暂未选择" && this.bqmz !== "暂未选择" && this.constent !== "" && this.imgsrc !== ""){
                 // 判断当文章修改为置顶以后,修改数据库拥有这个字段的元素为 false
                if(this.zda == true){
    ...
    created () {
        ...
        .then((res)=>{
            ...
            if(this.$store.state.contentnrs.zd == 'true'){
                // console.log('等于true')
                this.xs = false
                this.zda = true
            }else{
